...
class AppStore extends Component {
  state = {searchInput: '', isActiveTabId: tabsList[0].tabId}
  ...
  getCategoryRes = searchingcategory => {
    const {isActiveTabId} = this.state
    const filteredsearchres = searchingcategory.filter(
  ...
  render() {
    ...
    const searchResults = this.gettingSearchResults()
    const filteredsearchres = this.getCategoryRes(searchResults)
          ...
          <ul className="Tabs-list-container">
            {tabsList.map(eachItem => (
              <TabItem
                ...
                isActive={isActiveTabId === eachItem.tabId}
              />
            ))}
          </ul>
          <ul className="App-Item-container">
            {filteredsearchres.map(eachItem => (
